   ...
   <?php $this->load->view('subview/sidebar'); ?>
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Group of items & customize
                        <button class="btn btn-primary" onclick="javascript:load_addItem();">Add New group[+] </button> 
                        <button class="btn btn-primary" onclick="javascript:add_customize();">Customize item[+] </button> 
             ...
             <table id="example" class="display" cellspacing="0" width="100%">
                ...
                <?php foreach($amenities as $amenitiesItem) {?>
                    <tr>
                        <td><?php echo $amenitiesItem['item_id'];?></td>
                        <td><?php echo $amenitiesItem['item_name'];?></td>
                        <td><input type="text" id="edit_amenities_<?php echo $amenitiesItem['item_id'];?>" onchange="javascript:confirm_update('<?php echo $amenitiesItem['item_id'];?>')" name="edit_amenities" value="<?php echo $amenitiesItem['item_name'];?>" style="display:none"/>
                        <td><button class="btn btn-info"  onclick="javascript:load_update('<?php echo $amenitiesItem['item_id'];?>');">edit</button> 
                            <button class="btn btn-danger" onclick="javascript:confirm_delete('<?php echo $amenitiesItem['item_id'];?>')">delete</button>
                    </tr>
                    <?php } ?>
